...
│   ├── mobile/                  # React Native 직원 앱 (10개 화면)
...
| 기술 | 버전 | 용도 |
|------|------|------|
| React Native | v0.73 | 크로스 플랫폼 앱 |
| React Navigation | v6 | 네비게이션 |
| React Native Paper | v5.12 | Material Design UI |
| Firebase Messaging | v19 | 푸시 알림 |
| react-native-camera | v4.2 | 카메라 |
| react-native-image-picker | v7.1 | 이미지 선택 |
| @react-native-community/geolocation | v3.2 | GPS 위치 |
...
# Part 4: 모바일 앱 (packages/mobile)

## 14장. React Native 아키텍처

### 14.1 디렉토리 구조

```
packages/mobile/src/
├── App.tsx                         # 루트 컴포넌트 + 프로바이더
├── navigation/
│   └── AppNavigator.tsx            # 네비게이션 구조 정의
├── screens/
│   ├── auth/
│   │   ├── SplashScreen.tsx        # 앱 시작 (토큰 확인)
│   │   └── LoginScreen.tsx         # 로그인
│   ├── home/
│   │   └── HomeScreen.tsx          # 홈 (요약 정보)
│   ├── receipt/
│   │   ├── UploadScreen.tsx        # 1단계: 영수증 촬영/선택
│   │   ├── ProcessingScreen.tsx    # 2단계: OCR 처리 중
│   │   ├── ReceiptConfirmScreen.tsx# 3단계: OCR 결과 확인/수정
│   │   └── ResultScreen.tsx        # 4단계: 제출 완료
│   ├── transactions/
│   │   ├── TransactionListScreen.tsx   # 거래 내역 목록
│   │   └── TransactionDetailScreen.tsx # 거래 상세
│   └── settings/
│       └── SettingsScreen.tsx      # 설정 (비밀번호 변경, 로그아웃)
├── stores/
│   ├── authStore.ts                # 인증 상태 (Zustand + AsyncStorage)
│   ├── transactionStore.ts         # 거래 상태
│   └── settingStore.ts             # 앱 설정
├── services/
│   ├── api/
...
├── hooks/
│   ├── useAuth.ts                  # 인증 훅
│   ├── useCamera.ts                # 카메라 훅 (react-native-camera)
│   ├── useLocation.ts              # GPS 위치 훅
│   └── usePushNotification.ts      # FCM 푸시 알림 훅
├── components/                     # 재사용 컴포넌트
├── types/                          # 타입 정의
└── constants/                      # 상수
```
...
```typescript
// hooks/useCamera.ts
// react-native-camera + react-native-image-picker
// 촬영 또는 갤러리에서 이미지 선택
// 결과: { uri, type, fileName, fileSize }
```
...
// FCM 토큰 서버 전송 → employees.fcm_token 저장
// 거래 승인/반려 시 푸시 알림 수신
```

### 16.4 네이티브 빌드 주의사항

- `android/`, `ios/` 폴더가 초기화되어 있지 않음
- 빌드 전 `npx react-native init` 또는 네이티브 프로젝트 생성 필요
- Windows에서는 Android만 빌드 가능 (iOS는 macOS 필요)
- 필수: Android Studio + JDK 17+
...
### 26.5 모바일 앱 네이티브 초기화 필요

`packages/mobile/android/`, `ios/` 폴더가 비어있습니다. React Native CLI로 네이티브 프로젝트를 초기화해야 합니다.
...
| 모바일 진입점 | `packages/mobile/src/App.tsx` |
| 네비게이션 | `packages/mobile/src/navigation/AppNavigator.tsx` |
...
- React Native 0.73 → 0.74+: New Architecture 변경 확인
